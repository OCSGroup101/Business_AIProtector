...
//   1. IOC match rules  (LMDB lookup)
//   2. Behavioral rules (field condition matching)
//   3. Lua heuristics   (sliding window correlation)
//
// Detected events are augmented with DetectionHit records and dispatched
// to the containment module and assistant.
...
impl DetectionEngine {
    ...
    async fn process_event(&mut self, mut event: TelemetryEvent) {
        ...
        for (rule_id, compiled) in &rules.rules {
            if let Some(hit) = self.evaluate_rule(compiled, &event).await {
                debug!(
                    rule = %rule_id,
        ...
        if !hits.is_empty() {
            // Attach MITRE tags and detection results to event
            for hit in &hits {
            ...
            // Fast-path: send annotated event to AlertUploader → platform → incident_service
            ...
            // Dispatch containment for highest severity hit
    ...
    async fn evaluate_rule(
        ...
        let matched = match rule.match_block.match_type {
            ...
            MatchType::Behavioral => self.evaluate_behavioral_rule(rule, event),
            MatchType::Heuristic => {
    ...
    fn evaluate_ioc_rule(&self, rule: &DetectionRule, event: &TelemetryEvent) -> bool {
        for condition in &rule.match_block.conditions {
            if condition.operator == "in_ioc_set" {
                ...
                let value = get_field_value(event, &condition.field);
                if let Some(v) = value {
    ...
    fn evaluate_behavioral_rule(&self, rule: &DetectionRule, event: &TelemetryEvent) -> bool {
...
/// e.g. "payload.process_name" → looks up event.payload["process_name"]
